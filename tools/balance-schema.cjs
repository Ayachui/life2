...
const PARAMS = [
  ...
  p("economy.arcadeEconomy.upkeep.perExtra", "upkeep за лишнего", "Списывается floor(лишние × это), но не больше max. Только если задан потолок запаса ⚡.", { sheet: "Экономика_пульса" }),
  ...
  p("species.mutationChance.koala", "шанс коалы", "Шанс мутации зайца в коалу. Равен шансу коровы, снижен: коала теперь стачивает лес.", { sheet: "Мутации" }),
  p("species.mutationChance.cow", "шанс коровы", "Шанс мутации зайца в корову. Как у коалы.", { sheet: "Мутации" }),
  ...
  p("species.behavior.wolfCowPriority", "приоритет коровы", "Насколько волк предпочитает корову зайцу. 12 ≈ любая корова в поле зрения важнее ближайшего зайца.", { sheet: "Поведение" }),
  ...
  p("species.behavior.koalaTreeDowngradeGens", "стачивание дерева", "Сколько циклов коала сидит на дереве, прежде чем оно станет кустом. Потом идёт к следующему дереву.", { sheet: "Поведение", unit: "циклов" }),
  ...
  p("ecology.plants.bushEnergyPerBite", "сытость с куста", "Энергия за один укус куста (зайцы и прочие).", { sheet: "Растения" }),
  p("ecology.plants.bushBitesPerTickCow", "укусы коровы/цикл", "Сколько укусов куста корова делает за цикл. 2 укуса × 4 запаса = куст за 2 цикла (вдвое быстрее старого дерева).", { sheet: "Растения" }),
  p("ecology.plants.bushEnergyPerBiteCow", "сытость коровы с куста", "Энергия коровы за укус куста. 4×2.5 = 10 с куста.", { sheet: "Растения" }),
  ...
  p("ecology.plants.treeEnergyPerBiteKoala", "сытость коалы", "Энергия коалы за укус листвы, пока она стачивает дерево в куст.", { sheet: "Растения" }),
  ...
  p("ecology.cowManure.ttl", "навоз коровы TTL", "Сколько циклов лежит удобрение на месте съеденного куста.", { sheet: "Поле", unit: "циклов" }),
  p("ecology.cowManure.strength", "навоз коровы %", "Бонус роста растений вокруг (0.5 = +50%).", { sheet: "Поле" }),
  p("ecology.cowManure.radius", "навоз коровы R", "Радиус «вокруг» в клетках (1 = клетка и 8 соседей).", { sheet: "Поле", unit: "клеток" }),
  ...
  p("arcade.arcadeEnd.chainSustainGens", "цепочка", "Столько циклов подряд с зайцами = устойчивая цепочка (открывает медведя).", { sheet: "Конец_раунда", unit: "циклов" }),
